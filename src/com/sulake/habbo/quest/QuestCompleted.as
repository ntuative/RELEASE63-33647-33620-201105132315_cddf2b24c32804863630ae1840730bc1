package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private static const const_1488:int = 2000;
      ...
      private function onNextQuest(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._window.visible = false;
         if(this.var_321.completedQuestsInCampaign >= this.var_321.questCountInCampaign)
         {
            this.var_321.id = 0;
            this.var_321.accepted = false;
            this._questEngine.questController.campaignCompleted.show(this.var_321);
         }
         else
         {
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_321.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_321.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_321.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         var _loc3_:* = this.var_321.getQuestLocalizationKey() + ".completed";
      ...
      private function onCatalogLink(param1:WindowEvent, param2:IWindow = null) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            this._questEngine.openCatalog(this.var_321);
         }
      }
