package com.sulake.habbo.sound.trax
{
   ...
   public class TraxSequencer implements IHabboSound, IDisposable
   {
      ...
      private static const const_1368:uint = 88000;
      
      private static const const_1369:uint = 88000;
      
      private static const const_296:Vector.<int> = new Vector.<int>(const_186,true);
      ...
      private var var_899:Sound;
      
      private var var_418:SoundChannel;
      
      private var var_1124:TraxData;
      ...
      private var var_348:uint;
      
      private var var_1390:uint;
      ...
      {
         this.var_1390 = uint(30);
         super();
         this._events = param4;
         this.var_1688 = param1;
         this.var_754 = 1;
         this.var_899 = new Sound();
         this.var_418 = null;
      ...
      {
         return this.var_417 / const_113;
      }
      
      public function set fadeOutSeconds(param1:Number) : void
      {
         this.var_417 = int(param1 * const_113);
      }
      
      public function get fadeInSeconds() : Number
      {
         return this.var_728 / const_113;
      }
      
      public function set fadeInSeconds(param1:Number) : void
      {
         this.var_728 = int(param1 * const_113);
      }
      ...
      private function prepareSequence() : Boolean
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.var_1124.channels.length)
         {
            _loc3_ = new Map();
            _loc4_ = this.var_1124.channels[_loc2_] as TraxChannel;
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            while(_loc7_ < _loc4_.itemCount)
            {
               ...
               _loc9_ = this.var_1686.getValue(_loc8_) as TraxSample;
               _loc9_.setUsageFromSong(this.var_1688,_loc1_);
               if(_loc9_ == null)
               {
               ...
               while(_loc12_ < _loc11_)
               {
                  ...
                  _loc6_ += _loc10_;
                  _loc5_ = uint(_loc6_ * const_1369);
                  _loc12_++;
               }
               if(this.var_348 < _loc5_)
               {
                  this.var_348 = _loc5_;
               }
               _loc7_++;
      ...
      public function play(param1:Number = 0.0) : Boolean
      {
         ...
         this.var_496 = 0;
         this.var_588 = false;
         this.var_899.addEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         this.var_1391 = param1 * const_113;
         this.var_1123 = 0;
         this.var_1687 = 0;
         this.var_418 = this.var_899.play();
      ...
      {
         if(this.var_417 > 0 && !this.var_588)
         {
      ...
      private function stopImmediately() : void
      {
         ...
         {
            this.var_899.removeEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         }
      }
      ...
      private function stopWithFadeout() : void
      {
         if(this.var_589 == null)
         {
            this.var_727 = true;
            this.var_496 = 0;
            this.var_589 = new Timer(this.var_1390 + this.var_417 / (const_113 / 1000),1);
            this.var_589.start();
            this.var_589.addEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
         }
      }
      
      private function getSampleBars(param1:uint) : int
      {
         return Math.ceil(param1 / const_1368);
      }
      ...
      private function mixChannelsIntoBuffer() : void
      {
         ...
         var _loc2_:int = this.var_419.length;
         var _loc3_:* = null;
         var _loc4_:int = _loc2_ - 1;
         while(_loc4_ >= 0)
         {
            _loc5_ = this.var_419[_loc4_];
            ...
            _loc8_ = const_186;
            if(this.var_348 - this.var_199 < _loc8_)
            {
               _loc8_ = this.var_348 - this.var_199;
            }
            _loc9_ = 0;
            while(_loc9_ < _loc8_)
            {
      ...
      {
         var _loc1_:int = this.var_348 < this.var_1391 ? int(this.var_348) : (this.var_1391 > 0 ? int(this.var_1391) : int(this.var_348));
         if(this.var_199 > _loc1_ + this.var_1390 * (const_113 / 1000) && !this.var_588)
         {
            this.var_588 = true;
            if(this.var_416 != null)
            {
               this.var_416.reset();
               this.var_416.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
            }
            this.var_416 = new Timer(2,1);
            this.var_416.start();
            this.var_416.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
         }
         else if(this.var_199 > _loc1_ - this.var_417 && !this.var_727)
         {
            this.var_901 = false;
            this.var_727 = true;
            this.var_496 = 0;
      ...
      {
         if(param1.position > this.var_1123)
         {
            ++this.var_1687;
            Logger.log("Audio buffer under run...");
            this.var_1123 = param1.position;
         }
         if(this.volume > 0)
         {
         ...
         if(this.var_348 - this.var_199 < _loc2_)
         {
            _loc2_ = this.var_348 - this.var_199;
            if(_loc2_ < 0)
            {
               _loc2_ = 0;
            }
         ...
         this.var_199 += const_186;
         this.var_1123 += const_186;
         if(this.var_418 != null)
         {
            this.var_1390 = param1.position / const_113 * 1000 - this.var_418.position;
         }
      ...
      private function writeAudioToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         if(param2 > 0)
         {
            ...
            else
            {
               if(this.var_901)
               {
                  _loc5_ = 1 / this.var_728;
                  _loc6_ = this.var_900 / Number(this.var_728);
                  this.var_900 += const_186;
                  if(this.var_900 > this.var_728)
                  {
                     this.var_901 = false;
                  }
               }
               else if(this.var_727)
               {
                  _loc5_ = -1 / this.var_417;
                  _loc6_ = 1 - this.var_496 / Number(this.var_417);
                  this.var_496 += const_186;
                  if(this.var_496 > this.var_417)
                  {
                     this.var_496 = this.var_417;
                  }
               }
      ...
      {
         this.stopImmediately();
         this._events.dispatchEvent(new SoundCompleteEvent(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.var_1688));
      }
      ...
      private function removeFadeoutStopTimer() : void
      {
         if(this.var_589 != null)
         {
            this.var_589.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
            this.var_589.reset();
            this.var_589 = null;
         }
      }
