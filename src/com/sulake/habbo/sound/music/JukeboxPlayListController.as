package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_720 = [];
         this.var_720.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_720.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_720.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         this.var_230.stop(this.priority);
         this.var_462 = -1;
         this.var_1668 = -1;
         this.var_683 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         {
            this.var_230.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_129,_loc3_.syncCount / 1000,0,1,1);
            this.var_462 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_230.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1668 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_99,HabboMusicPrioritiesEnum.const_129,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_230.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_721.indexOf(_loc5_) < 0)
               {
                  this.var_721.push(_loc5_);
                  this.var_230.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_493.push(_loc7_);
            _loc4_++;
         }
         if(this.var_721.length == 0)
         {
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_493[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_230.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_493[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_721.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_721.splice(_loc3_,1);
         }
         if(this.var_721.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_103));
