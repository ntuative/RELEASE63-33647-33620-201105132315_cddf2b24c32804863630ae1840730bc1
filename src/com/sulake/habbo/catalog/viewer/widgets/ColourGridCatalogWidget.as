package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private var var_345:IItemGridWindow;
      ...
      {
         if(!this.var_345.disposed)
         {
            this.var_345.destroyGridItems();
      ...
      override public function init() : Boolean
      {
         ...
         this.var_345 = _window.findChildByName("colourGrid") as IItemGridWindow;
         var _loc1_:XmlAsset = page.viewer.catalog.assets.getAssetByName("color_chooser_cell") as XmlAsset;
         this.var_1642 = _loc1_.content as XML;
      ...
      private function onAvailableMultiColours(param1:CatalogWidgetMultiColoursEvent) : void
      {
         ...
         for each(_loc2_ in param1.colours)
         {
            this.var_891.push(_loc2_.slice());
         }
         _loc3_ = page.viewer.catalog.assets.getAssetByName(param1.backgroundAssetName) as BitmapDataAsset;
         this.var_580 = _loc3_.content as BitmapData;
         _loc4_ = page.viewer.catalog.assets.getAssetByName(param1.colourAssetName) as BitmapDataAsset;
         ...
         this.var_889 = _loc5_.content as BitmapData;
         this.populateColourGrid();
         this.select(this.var_345.getGridItemAt(0) as IWindowContainer);
      }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_216.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_891.length,this.var_345.numGridItems]);
         this.var_345.destroyGridItems();
         this.var_216 = null;
         for each(_loc1_ in this.var_891)
         {
            if(_loc1_.length > 0)
            {
               _loc2_ = page.viewer.catalog.windowManager.buildFromXML(this.var_1642) as IWindowContainer;
               _loc2_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onClick);
               _loc2_.background = true;
               _loc2_.color = 4294967295;
               _loc2_.width = this.var_580.width;
               _loc2_.height = this.var_580.height;
               this.var_345.addGridItem(_loc2_);
               _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
               if(_loc3_ != null)
               {
                  _loc3_.bitmap = new BitmapData(this.var_580.width,this.var_580.height,true,0);
                  _loc3_.bitmap.copyPixels(this.var_580,this.var_580.rect,new Point(0,0));
               }
               _loc4_ = _loc2_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
               if(_loc4_ != null)
               {
                  _loc4_.bitmap = new BitmapData(this.var_890.width,this.var_890.height,true,0);
                  _loc6_ = 255;
                  _loc7_ = 255;
                  _loc8_ = 255;
                  _loc9_ = null;
               ...
               if(_loc5_ != null)
               {
                  _loc5_.bitmap = new BitmapData(this.var_889.width,this.var_889.height,true,16777215);
                  _loc5_.bitmap.copyPixels(this.var_889,this.var_889.rect,new Point(0,0),null,null,true);
      ...
      private function onClick(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:int = this.var_345.getGridItemIndex(param1.target as IWindow);
