package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1239:Number = 1.0;
      
      private var var_644:BitmapData;
      
      private var var_798:ColorTransform;
      
      private var var_1508:ColorTransform;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1508 = new ColorTransform();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_103 = this.var_1238[param1];
         this.var_999 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1899 >= 0)
         {
            this.var_267 = this._visualization.getSprite(this.var_1899);
            if(this.var_267 && this.var_267.asset)
            {
               ...
               if(this.var_89 == null)
               {
                  this.var_89 = this.var_267.asset.clone();
                  if(this.var_798.alphaMultiplier != 1)
                  {
                     this.var_644 = new BitmapData(this.var_89.width,this.var_89.height,true,4278190080);
                  }
               }
               this.var_2650 = -this.var_267.offsetX;
               this.var_2648 = -this.var_267.offsetY;
               this.var_267.asset = this.var_89;
            }
            if(this.var_89)
            {
            ...
            if(this.var_644)
            {
               this.var_644.fillRect(this.var_644.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_103.y * this.var_1239;
         }
         return 0;
      }
      ...
      {
         if(!this.var_103 || !this.var_267)
         {
            return;
         }
         if(this.var_89 && this.var_267.asset != this.var_89)
         {
            this.var_267.asset = this.var_89;
         }
         if(this.var_999)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_999 && this.var_103.hasIgnited)
         {
            this.var_999 = true;
         }
         _loc8_ = this.var_1155 * this.var_1239;
         this.var_103.update();
         if(this.var_999)
         {
            if(this.var_103.roomObjectSpriteId >= 0)
            ...
            {
               this.var_89.draw(this.var_644,this.var_1898,this.var_798,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_103.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2650 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1239;
               _loc4_ = this.var_2648 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1239;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1508.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_89.draw(_loc7_,this._translationMatrix,this.var_1508,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2649 = parseInt(param1.@size);
         this.var_1899 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1155 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1239 = this.var_2649 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_798.alphaMultiplier = this._blend;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "RoomWidgetStopEffectMessage" ? true : false;
               _loc21_ = [];
