package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1943:String = "no_reqs_info";
      ...
      {
         this.var_1886 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         this.var_1885 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         this.var_1885.procedure = this.onDismissAllButtonClick;
         this.var_1886.procedure = this.onAcceptAllButtonClick;
         this.refreshButtons();
      }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         if(this._friendList && this._friendList.events)
         {
            this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_309,param1));
         }
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_407 && _loc4_.state != FriendRequest.const_430)
            {
               _loc4_.state = FriendRequest.const_430;
               this.refreshRequestEntry(_loc4_);
               if(this._friendList && this._friendList.events)
               {
                  this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_309,_loc4_.requestId));
               }
            }
         }
         this.refresh();
